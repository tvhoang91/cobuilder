...
export default function ProjectCombobox({ currentProjectSlug }: ProjectComboboxProps) {
  ...
  const currentProject = currentProjectSlug ? projects.find((project) => project.slug === currentProjectSlug) : null
            ...
            <CommandGroup>
              {projects.map((project) => (
                <CommandItem key={project.id} value={project.slug} onSelect={() => handleProjectSelect(project.slug)}>
                  <Check className={cn('h-4 w-4', currentProjectSlug === project.slug ? 'opacity-100' : 'opacity-0')} />
